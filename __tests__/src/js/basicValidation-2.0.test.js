...
describe('Spectral Validation Rules', () => {
  ...
  test('should fail validation for Swagger 2.0 - Part 1', async () => {
    ...
    const expectedResults = [
      ...
      ['oas2-schema', Severity.Error, '"post" property must have required property "responses".', '/paths/~1users~1{userId}/post'],
      ['oas2-schema', Severity.Error, '"tags" property must not have duplicate items (items ## 1 and 2 are identical).', '/tags'],
      ['operation-operationId-unique', Severity.Error, 'Every operation must have unique "operationId".', '/paths/~1users~1{userId}/post/operationId'],
      ...
      ['path-params', Severity.Error, 'Operation must define parameter "{userId}" as expected by path "/users/{userId}".', '/paths/~1users~1{userId}/get'],
      ['path-params', Severity.Error, 'Operation must define parameter "{userId}" as expected by path "/users/{userId}".', '/paths/~1users~1{userId}/post'],
      ['path-params', Severity.Error, 'Parameter "unusedPath" must be used in path "/items".', '/paths/~1items/parameters/0'],
      ['path-params', Severity.Error, 'Paths "/users/{id}" and "/users/{userId}" must not be equivalent.', '/paths/~1users~1{userId}'],
      ...
      ['no-script-tags-in-markdown', Severity.Warning, 'Markdown descriptions must not have "<script>" tags.', '/paths/~1users~1{userId}/get/description'],
      ...
      ['operation-description', Severity.Warning, 'Operation "description" must be present and non-empty string.', '/paths/~1users~1{userId}/post'],
